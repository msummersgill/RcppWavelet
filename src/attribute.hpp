...
#pragma mark === Functions checkLimits ===
    ...
    /**
     * @brief checks the validity of the requested value with respect to the current limits
     * @param value requested attribute value
     * @param limit_min minimum value
     * @param limit_max maximum value
     */
    template <typename T>
    void checkLimits(T const& value,
                     T const& limit_min,
                     T const& limit_max)
    {
        throw std::runtime_error("Attribute limits are not implemented for the current type.");
    }
    ...
    template <>
    void checkLimits<char>(char const& value,
    ...
    template <>
    void checkLimits<int>(int const& value,
                          int const& limit_min,
                          int const& limit_max);
    
    template <>
    void checkLimits<long>(long const& value,
                           long const& limit_min,
                           long const& limit_max);
    
    template <>
    void checkLimits<float>(float const& value,
                            float const& limit_min,
                            float const& limit_max);
    
    template <>
    void checkLimits<double>(double const& value,
...
#pragma mark === Class AttributeHandler ===
    ...
    class AttributeHandler {
        template <typename T> friend class Attribute;
...
#pragma mark === Class Attribute ===
    ...
    public:
         ...
         */
        Attribute(AttributeHandler *parent=nullptr,
                  T const& value=T(),
         ...
         * @throws runtime_error exception if the limit checking are not implemented for the current type
         */
        void set(T const& value, bool silently = false)
        {
            ...
            changed = true;
            if (!silently && parent_)
                parent_->onAttributeChange(this);
        }
